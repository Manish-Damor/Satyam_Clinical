...
class TestFramework {
    ...
    public function assertDatabaseHasRecord($table, $where, $testName, $message = "") {
        try {
            if (!$this->tableExists($table)) {
                $this->recordFail($testName, "Table not found: $table");
                return false;
            }
            ...
            foreach ($where as $column => $value) {
                if ($value === null) {
                    ...
                    $safeValue = $this->connect->real_escape_string((string)$value);
    ...
    public function tableExists($tableName) {
        $safe = $this->connect->real_escape_string((string)$tableName);
        $result = $this->connect->query("SHOW TABLES LIKE '" . $safe . "'");
     ...
     */
    public function findExistingTable($candidates) {
