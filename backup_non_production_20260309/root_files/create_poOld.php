    ...
    document.querySelectorAll('.item-row').forEach(row => {
        ...
        if(medicineId && quantity > 0) {
            items.push({
                ...
                medicine_name: row.querySelector('.medicine-search').value,
                quantity: quantity,
                ...
                tax_percent: parseFloat(row.querySelector('.tax-percent').value) || 18,
                line_amount: parseFloat(row.querySelector('.line-amount').value) || 0,
                item_total: parseFloat(row.querySelector('.item-total').value) || 0
            });
        }
    });
...
console.log("JSON STRING:", JSON.stringify(formData));
    ...
    fetch('php_action/createPurchaseOrder.php', {
        method: 'POST',
    ...
    alert('Server error while creating PO. Check logs.');
});
