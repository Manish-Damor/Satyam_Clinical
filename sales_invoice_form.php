                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="card" style="background-color: #f9f9f9; padding: 15px; min-height: 120px;">
                                    <div id="billToAddress" style="white-space: pre-wrap;">
                        ...
                        <table class="table table-bordered" id="invoiceItemsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 8%;" class="no-print">PTR</th>
                                    ...
                                    <td>
                                        <div class="form-group" style="position: relative;">
                                            ...
                                            <div class="search-results" style="position: absolute; background: white; border: 1px solid #ddd; 
                                                max-height: 250px; overflow-y: auto; display: none; z-index: 100; width: 100%; border-radius: 4px;"></div>
                                        </div>
                                    </td>
                                    <td>
                                        <select class="form-control batch-select" name="batch_id[]" data-row="1">
                                            <option value="">Select Batch</option>
                                        </select>
                                    </td>
                                    <td>
                                        <span class="available-qty" id="avail1">-</span>
                                    </td>
                                    <td>
                                        <input type="number" class="form-control quantity-input" name="quantity[]" 
                                            data-row="1" min="1" step="0.01" />
                                    </td>
                                    <td>
                                        <input type="text" class="form-control rate-display" readonly data-row="1" />
                                        <input type="hidden" class="rate-value" name="rate[]" />
                                    </td>
                                    <td class="no-print">
                                        <input type="text" class="form-control ptr-display" readonly data-row="1" />
                                        <input type="hidden" class="ptr-value" name="ptr[]" />
                                    </td>
                                    <td>
                                        <input type="number" class="form-control gst-input" name="gst_rate[]" 
                                            value="18" step="0.01" />
                                    </td>
                                    <td>
                                        <input type="text" class="form-control total-display" readonly data-row="1" />
                                        <input type="hidden" class="total-value" name="line_total[]" />
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-danger remove-row" data-row="1">
...
$(document).ready(function() {
    ...
    $(document).on('keyup', '.product-search', function() {
        ...
        $.ajax({
            ...
            success: function(products) {
                const resultsDiv = $(`#row${rowNum} .search-results`);
                resultsDiv.empty();
                ...
                if (products.length === 0) {
                    ...
                    products.forEach(product => {
                        const item = $(`<div style="padding: 8px; cursor: pointer; border-bottom: 1px solid #eee;" class="product-item" data-row="${rowNum}" data-id="${product.id}" data-name="${product.product_name}" data-price="${product.expected_mrp}">${product.text}</div>`);
    ...
    $(document).on('click', '.product-item', function() {
        ...
        $(`#row${rowNum} .product-search`).val(productName);
        $(`#row${rowNum} .product-id`).val(productId);
        $(`#row${rowNum} .search-results`).hide();
        
        // Fetch product details
        fetchProductDetails(rowNum, productId);
    });
    
    // Quantity change - recalculate
    $(document).on('change', '.quantity-input', function() {
        const rowNum = $(this).data('row');
        calculateLineTotal(rowNum);
    });
...
function addInvoiceRow() {
    ...
    const row = `<tr id="row${rowCount}" class="item-row">
        ...
        <td>
            <select class="form-control batch-select" name="batch_id[]" data-row="${rowCount}">
                <option value="">Select Batch</option>
            </select>
        </td>
        <td>
            <span class="available-qty" id="avail${rowCount}">-</span>
        </td>
        <td>
            <input type="number" class="form-control quantity-input" name="quantity[]" 
                data-row="${rowCount}" min="1" step="0.01" />
        </td>
        <td>
            <input type="text" class="form-control rate-display" readonly data-row="${rowCount}" />
            <input type="hidden" class="rate-value" name="rate[]" />
        </td>
        <td class="no-print">
            <input type="text" class="form-control ptr-display" readonly data-row="${rowCount}" />
            <input type="hidden" class="ptr-value" name="ptr[]" />
        </td>
        <td>
            <input type="number" class="form-control gst-input" name="gst_rate[]" 
        ...
        <td>
            <input type="text" class="form-control total-display" readonly data-row="${rowCount}" />
            <input type="hidden" class="total-value" name="line_total[]" />
        </td>
        <td>
            <button type="button" class="btn btn-sm btn-danger remove-row" data-row="${rowCount}">
                <i class="fa fa-trash"></i>
            </button>
        </td>
    </tr>`;
...
function fetchProductDetails(rowNum, productId) {
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Set rate and PTR
                $(`#row${rowNum} .rate-display`).val(product.selling_price);
                $(`#row${rowNum} .rate-value`).val(product.selling_price);
                $(`#row${rowNum} .ptr-display`).val(product.ptr || 'N/A');
                $(`#row${rowNum} .ptr-value`).val(product.ptr || 0);
                $(`#row${rowNum} .gst-input`).val(product.gst_rate);
                
                // Populate batches
                const batchSelect = $(`#row${rowNum} .batch-select`);
                batchSelect.empty();
                batchSelect.append('<option value="">Select Batch</option>');
                
                batches.forEach(batch => {
                    const expiryDate = new Date(batch.expiry_date).toLocaleDateString();
                    batchSelect.append(`<option value="${batch.batch_id}" data-available="${batch.available_quantity}">
...
function calculateLineTotal(rowNum) {
    const quantity = parseFloat($(`#row${rowNum} .quantity-input`).val()) || 0;
    const rate = parseFloat($(`#row${rowNum} .rate-value`).val()) || 0;
    const gstRate = parseFloat($(`#row${rowNum} .gst-input`).val()) || 0;
    
    const lineSubtotal = quantity * rate;
    const gstAmount = lineSubtotal * (gstRate / 100);
    const lineTotal = lineSubtotal + gstAmount;
    
    $(`#row${rowNum} .total-display`).val(lineTotal.toFixed(2));
    $(`#row${rowNum} .total-value`).val(lineTotal.toFixed(2));
...
function calculateTotals() {
    let subtotal = 0;
    $('input.total-value').each(function() {
    ...
    // For simplicity, calculate GST on after-discount amount
    const gstAmount = afterDiscount * 0.18; // Assuming 18% GST
    const grandTotal = afterDiscount + gstAmount;
    
    $('#subtotal').val(subtotal.toFixed(2));
    $('#subtotalValue').val(subtotal.toFixed(2));
    $('#discountAmount').val(discountAmount.toFixed(2));
    $('#discountAmountValue').val(discountAmount.toFixed(2));
    $('#gstAmount').val(gstAmount.toFixed(2));
    $('#gstAmountValue').val(gstAmount.toFixed(2));
