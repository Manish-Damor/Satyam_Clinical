...
function get_invoice_row($connect, $invoiceId) {
    $stmt = $connect->prepare("SELECT id, status, paid_amount FROM purchase_invoices WHERE id = ? LIMIT 1");
    if (!$stmt) return null;
...
function has_invoice_stock_postings($connect, $invoiceId) {
    $referenceNumber = (string)$invoiceId;
    $stmt = $connect->prepare("SELECT COUNT(*) AS cnt FROM stock_movements WHERE reference_type = 'purchase_invoice' AND (reference_id = ? OR reference_number = ?)");
    if (!$stmt) {
        return true;
    }
    $stmt->bind_param('is', $invoiceId, $referenceNumber);
    $stmt->execute();
    $res = $stmt->get_result();
...
// ============================================
    ...
    $targetInvoiceId = intval($invoiceId ?? 0);
    ...
    $inv = get_invoice_row($connect, $targetInvoiceId);
    if (!$inv) {
    ...
    // Only Draft invoices can be approved in this flow.
    if ($currentStatus !== 'Draft') {
        echo json_encode([
            'success' => false, 
    ...
    $ok = PurchaseInvoiceAction::approveInvoice($targetInvoiceId, $_SESSION['userId'] ?? 0);
    if ($ok) {
        echo json_encode([
            'success' => true, 
...
// ============================================
    ...
    // Restrict deletion to Draft only to protect stock integrity and audit trail.
    if ($currentStatus !== 'Draft') {
        echo json_encode(['success' => false, 'error' => 'Only Draft invoices can be deleted']);
        exit;
    }

    if (abs($paidAmount) > 0.01) {
        echo json_encode(['success' => false, 'error' => 'Cannot delete invoice with payment entries']);
        exit;
    }

    if (has_invoice_stock_postings($connect, $targetInvoiceId)) {
        echo json_encode(['success' => false, 'error' => 'Cannot delete invoice with stock postings']);
        exit;
    }

    $connect->begin_transaction();

    $deleteStmt = $connect->prepare("DELETE FROM purchase_invoices WHERE id = ? AND status = 'Draft'");
    if (!$deleteStmt) {
        $connect->rollback();
    ...
    $deleteStmt->bind_param('i', $targetInvoiceId);
    if ($deleteStmt->execute()) {
        ...
        if ($deletedRows !== 1) {
            $connect->rollback();
            echo json_encode(['success' => false, 'error' => 'Invoice was not deleted']);
            exit;
        }
