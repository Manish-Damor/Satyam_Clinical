...
if ((strtotime($filterEndDate) - strtotime($filterStartDate)) / 86400 > 365) {
    $filterStartDate = date('Y-m-d', strtotime($filterEndDate . ' -365 days'));
}
...
$purchaseInRange = (float) getSingleValue($connect, "
...
$salesInRange = (float) getSingleValue($connect, "
        SELECT COALESCE(SUM(grand_total), 0)
        FROM sales_invoices
...
$salesCollectionInRange = (float) getSingleValue($connect, "
...
$estimatedSalesMargin = (float) getSingleValue($connect, "
        SELECT COALESCE(SUM(sii.line_total - (COALESCE(sii.purchase_rate, 0) * sii.quantity)), 0)
...
$salesCollectionRate = $salesInRange > 0 ? (($salesCollectionInRange / $salesInRange) * 100) : 0;

$pendingPurchaseInvoices = (int) getSingleValue($connect, "
    SELECT COUNT(*)
    FROM purchase_invoices
        WHERE deleted_at IS NULL
            AND (status IN ('Draft', 'Approved', 'Received') OR status IS NULL)
      AND invoice_date BETWEEN '{$startEsc}' AND '{$endEsc}'
", 0);

$supplierPayables = (float) getSingleValue($connect, "
        SELECT COALESCE(SUM(COALESCE(outstanding_amount, grand_total - COALESCE(paid_amount, 0))), 0)
    FROM purchase_invoices
        WHERE deleted_at IS NULL
            AND COALESCE(status, '') <> 'Cancelled'
            AND invoice_date BETWEEN '{$startEsc}' AND '{$endEsc}'
...
$supplierOverdue = (float) getSingleValue($connect, "
        SELECT COALESCE(SUM(COALESCE(outstanding_amount, grand_total - COALESCE(paid_amount, 0))), 0)
        FROM purchase_invoices
        WHERE deleted_at IS NULL
            AND COALESCE(status, '') <> 'Cancelled'
            AND due_date < CURDATE()
            AND COALESCE(outstanding_amount, grand_total - COALESCE(paid_amount, 0)) > 0
", 0);
...
$customerOverdue = (float) getSingleValue($connect, "
    SELECT COALESCE(SUM(COALESCE(due_amount, grand_total - COALESCE(paid_amount, 0))), 0)
    FROM sales_invoices
        WHERE due_date < CURDATE()
            AND COALESCE(due_amount, grand_total - COALESCE(paid_amount, 0)) > 0
            AND (is_cancelled IS NULL OR is_cancelled = 0)
", 0);
...
$recentTransactionsSql = "
    SELECT * FROM (
        ...
        WHERE po.po_date BETWEEN '{$startEsc}' AND '{$endEsc}'
                    AND COALESCE(po.delete_status, 0) = 0
        ...
        SELECT
            ...
            COALESCE(pi.payment_status, pi.status, '-') AS payment_status,
...
$overdueSalesSql = "
    SELECT si.invoice_number, c.name AS client_name, si.due_date, COALESCE(si.due_amount, si.grand_total - COALESCE(si.paid_amount, 0)) AS due_amount
    FROM sales_invoices si
    LEFT JOIN clients c ON c.client_id = si.client_id
    WHERE si.due_date < CURDATE()
            AND COALESCE(si.due_amount, si.grand_total - COALESCE(si.paid_amount, 0)) > 0
...
.quick-btn { min-width: 88px; margin-left: 4px; margin-bottom: 4px; }
</style>
    ...
    <div class="container-fluid">
        <div class="row page-titles">
            <div class="col-md-8 align-self-center">
                <h3 class="text-primary">Operations Dashboard</h3>
                <small class="text-muted">Purchases, sales, stock risk, and receivable/payable health (<?php echo htmlspecialchars(date('d M Y', strtotime($filterStartDate))); ?> - <?php echo htmlspecialchars(date('d M Y', strtotime($filterEndDate))); ?>)</small>
            </div>
            <div class="col-md-4 text-right align-self-center">
                ...
                <a href="sales_report.php" class="btn btn-sm btn-dark quick-btn"><i class="fa fa-line-chart"></i> Sales</a>
                <a href="inventory_reports.php" class="btn btn-sm btn-secondary quick-btn"><i class="fa fa-cubes"></i> Stock</a>
            </div>
        </div>
                ...
                <div class="card dashboard-filter-card">
                    <div class="card-body py-2">
                        ...
                        <div class="mt-2 text-muted" style="font-size:.85rem;">
                            Est. sales margin: <strong>₹<?php echo number_format($estimatedSalesMargin, 2); ?></strong>
                            | Customer overdue: <strong>₹<?php echo number_format($customerOverdue, 2); ?></strong>
                            | Supplier overdue: <strong>₹<?php echo number_format($supplierOverdue, 2); ?></strong>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-xl-2 col-lg-4 col-md-6 mb-3">
                <div class="card text-white bg-secondary kpi-card"><div class="card-body">
                    <div class="kpi-value">₹<?php echo number_format($purchaseInRange, 0); ?></div>
                    <div class="kpi-label">Purchase in Range</div>
                </div></div>
            </div>
            <div class="col-xl-2 col-lg-4 col-md-6 mb-3">
                <div class="card text-white bg-success kpi-card"><div class="card-body">
                    <div class="kpi-value">₹<?php echo number_format($salesInRange, 0); ?></div>
        ...
        <div class="row">
            ...
            <div class="col-xl-2 col-md-6 mb-3">
                <div class="card kpi-card"><div class="card-body">
                    <div class="h4 mb-1">₹<?php echo number_format($salesCollectionInRange, 0); ?></div>
                    <div class="text-muted">Collections in Range</div>
                </div></div>
            </div>
            <div class="col-xl-2 col-md-6 mb-3">
                <div class="card kpi-card"><div class="card-body">
                    <div class="h4 mb-1"><?php echo number_format($salesCollectionRate, 1); ?>%</div>
